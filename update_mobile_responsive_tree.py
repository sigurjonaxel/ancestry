...
# 1. Update index.css with dedicated mobile tree styling
with open("index.css", "r", encoding="utf-8") as f:
    css = f.read()

mobile_tree_css = '''
/* 📱 SÉRHÖNNUÐ SÍMASKJÁA-ÚTGÁFA FYRIR ÆTTARTRÉ */
@media (max-width: 768px) {
  ...
  .mobile-tree-card.focus {
    background: linear-gradient(135deg, rgba(184,134,11,0.2) 0%, rgba(20,24,30,0.9) 100%);
    border-color: var(--accent-gold);
    box-shadow: 0 0 10px rgba(184,134,11,0.3);
  }

  .mobile-tree-card:active {
...
if "📱 SÉRHÖNNUÐ SÍMASKJÁA-ÚTGÁFA" not in css:
    css += "\n" + mobile_tree_css
    with open("index.css", "w", encoding="utf-8") as f:
        f.write(css)

# 2. Update app.js to render both Desktop 2D Graph and Mobile Card Flow
with open("app.js", "r", encoding="utf-8") as f:
    js = f.read()

# Replace renderInteractiveTree with a smart hybrid renderer
new_tree_code = '''
// ==========================================
...
parts = js.split("// ==========================================\n// GLÆSILEGT GAGNVIRKT ÆTTARTRÉ")
if len(parts) > 1:
    js_before = parts[0]
    # Check if there is code after renderInteractiveTree
    after_parts = parts[1].split("// ==========================================\n// STÆKKUN OG MINNKUN Á PANELUM")
    js_after = "\n\n// ==========================================\n// STÆKKUN OG MINNKUN Á PANELUM" + after_parts[1] if len(after_parts) > 1 else ""
    js = js_before + new_tree_code + js_after
    with open("app.js", "w", encoding="utf-8") as f:
        f.write(js)
    print("✓ app.js uppfært með sjálfvirkri símaskjáaskynjun fyrir ættartré!")
else:
    print("Gat ekki splittað app.js")
